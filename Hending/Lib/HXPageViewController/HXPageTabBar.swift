...
@objc enum HXPageTabBarItemStyle: Int {
    /// 默认
    case `default`
    case bgImage
}
...
@objc protocol HXPageTabBarDataSource: class {
    ...
    /// 切换按钮的样式，默认为无背景图
    ///
    /// - Parameter pageTabBar: pageTabbar: pageTabBar
    /// - Returns: 切换按钮样式
    @objc optional func itemStyle(in pageTabBar: HXPageTabBar) -> HXPageTabBarItemStyle
...
class HXPageTabBar: UIView {
    ...
    weak var dataSource: HXPageTabBarDataSource? {
        didSet {
            if let defaultIndex = dataSource?.defaultSelectedIndex?(in: self) {
                selectedIndex = min(max(0, defaultIndex), max(0, numberOfItems() - 1))
            }
            refreshCurrentState()
        }
    }
    ...
    func clearMark(_ index: Int) {
        itemModels[index].number = 0
...
//            && itemStyle() == .default
        ...
        /// 如果没有title
        guard let titleCount = dataSource?.numberOfItems(in: self) else {
            return 0
        }
        if titleCount == 0 {
            return 0
        ...
        var width: CGFloat = 0
        if let font = font {
            width = HXPageTabBarUtil.stringWidth(with: title.title, font: font)
        }else {
            ...
            width = HXPageTabBarUtil.stringWidth(with: title.title, font: titleFont)
        }
        if itemStyle() == .bgImage {
            if width < 82 {
                return 82
    ...
    /// - Parameter index: 位置
    /// - Returns: 标题
    private func titleForItem(at index: Int) -> HDWarningModel {
        return dataSource?.pageTabBar(self, titleForItemAt: index) ?? HDWarningModel()
    }
    ...
    ///
    /// - Returns: 样式
    private func itemStyle() -> HXPageTabBarItemStyle {
        return dataSource?.itemStyle?(in: self) ?? .default
    }
...
extension HXPageTabBar {
    ...
    private func setupContentScrollObserver() {
        guard let contentScrollView = contentScrollView as? HXPageContentView  else { return }
        contentScrollView.scrollObserver = contentScrollView.observe(\.contentOffset, options: [.new, .old]) { [weak self] (scrollView, changed) in
            guard let `self` = self,
    ...
    private func itemModel(at index: Int, percent: CGFloat = 1) -> HXPageTabBarItemModel {
        let model = titleForItem(at: index)
        ...
        let style: HXPageTabBarItemStyle = itemStyle()
        ...
        let itemModel = HXPageTabBarItemModel(title: title,
                                              number: number,
                                              titleFont: titleFont,
                                              titleHighlightedFont: titleHighlightedFont,
